...
(function(){
  fname = path.join(__dirname, 'la.dic');
  var dicOut = '';
  for(var i in patLA.patterns) {
    var arr = patLA.patterns[i].match(new RegExp('.{1,'+i+'}','g'));
    dicOut += '\n'+arr.join('\n');
  }
  fs.writeFileSync(fname, dicOut);
})();
...
txt = 'coagulátum exstíngue frigus fíguli fúlgura gubernáre insúrgunt jugum largus linguam linguis lingua pinguédine regum sanguínibus singuláriter sánguinem ánguli árguit comedamus noster surr'+e1+'ximus red'+e1+'mptio custodíri transíbunt';
var htxt = hyph(txt)
console.log(htxt);
// */
